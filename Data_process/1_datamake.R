...
A2O <- function(ASVtable, cortab){ #- cortab <- sq97; ASVtable <- seqtab
  cor <- cortab[,4:ncol(cortab)]
  rownames(cor) <- cortab[,2]
...
tab99 <- A2O(seqtab, sq99); tab97 <- A2O(seqtab, sq97)

saveRDS(tab99, sprintf("Data/seqOTUtab_99.rds"))
saveRDS(tab97, sprintf("Data/seqOTUtab_97.rds"))
# -- #
...
number <- 3000 ## important!!!!!!!!!!!!!!!!!!!!!! the number of rarefaction
