...
library(ecoregime); library(grid); library(ppcor); library(lmodel2)
...
for(i in 6){ #- i=6
  ...
  pf <- function(us=uSil, dd, exp, col=Inocol, col2=Scol, leg=0){ #- dd=2; exp=1
    ...
    ma_results <- df %>%
      ...
      rowwise() %>%
      mutate(
        intercept = model$regression.results$Intercept[2],  # intercept
        slope = model$regression.results$Slope[2],  # slope
    ...
    ans <- ggplot(df, aes(x=x, y=y))+
      ...
      scale_fill_manual(values=col)+xlab("Initial variation<br>upon inoculation")+
  ...
  pl_A[[i]] <- plot_grid(pl_C[[i]], pl_D[[i]], ncol=2)
}

pdf(sprintf("%s/ForFig_Disc_and_Cont_All.pdf", fd), width=8, height=3.8)
pl_A[[6]]
dev.off()
