...
###################################
SilRess <- list()
qthr <- 0.05 #-- qthr is the threshold of qvalue for FDR
Readthr <- 3000; fititr <- 10 #-- Readthr is the thr of rarefaction、fititr is the iteration number for fitting
for(i in 5:7){ #- i=6
  ...
  for(j in 1:nrow(SilRes)){ #- j=2
    if(SilRes$qvalue[j] <= qthr){
      ...
      for(cl in 1:length(clus)){ #- cl=1
        ...
        if(nrow(ldf2) > 5){ #- using only the cluster with more than 5 samples
          itrs <- c(); itro <- c()
          for(itr in 1:100){
