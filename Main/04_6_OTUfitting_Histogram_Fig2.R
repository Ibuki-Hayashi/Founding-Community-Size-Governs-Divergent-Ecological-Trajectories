...
ksil <- function(vec){ #- vec <- ldf
  ...
  for(i in 2:6){ #- i=3
    ...
    sil[i] <- mean(s[, 3])  #- the average silhouette width for all samples, which is a measure of how well the samples are clustered. A higher silhouette width indicates better clustering.
  }
...
log_likelihood <- function(sigma, observed_data, n, p){ #- n=3000; p=1/3; sigma=130; observed_data=df
  ...
  return(-log_likelihood_value)  # negative log-likelihood
}

#-- Histogram
qthr <- 0.05 #-- significant threshold of qvalue for monomodal distribution; if qvalue > qthr, we consider it as monomodal distribution and fit the binomial distribution; otherwise, we consider it as multimodal distribution and fit the binomial distribution for each cluster.
Readthr <- 3000 #-- Threhold of reads for binomial distribution
histlist <- list()
for(i in 6){ #- i=6
  histlist[[i]] <- gList()
  SilRes <- Sil[[i]]
  SilRes <- SilRes[SilRes$Day == 2,]
  SilRes <- SilRes[(SilRes$Taxa == "X_003")&(SilRes$Source == "Water"),]
  for(j in 1:nrow(SilRes)){ #- j=1; j=260
    ...
    if(SilRes$qvalue[j] > qthr){
      ...
      histlist[[i]][[j]] <- ggplot(data.frame(Sp=ldf), aes(x=Sp))+
        geom_histogram(bins = 60, fill = "grey20", color = NA, alpha = 0.6, size=0.2)+
        theme_classic()+ggtitle(sprintf("%s_%s_%s", SilRes$Source[j], SilRes$Density[j], SilRes$Taxa[j]))+
      ...
      histlist[[i]][[j]] <- ggplot(data.frame(Sp=ldf), aes(x=Sp))+
        geom_histogram(bins = 60, fill = "grey20", color = NA, alpha = 0.6)+
        theme_classic()+ggtitle(sprintf("%s_%s_%s", SilRes$Source[j], SilRes$Density[j], SilRes$Taxa[j]))+
        theme(text=element_text(size=6),axis.title.y=element_markdown(),
              legend.position="none")+xlab("Relative abundance")+ylab("Frequency")+
        geom_line(data = theory_density, aes(x = x, y = y_scaled), color = "red", linewidth = 0.5)+
...
cc <- gList()
OTUL <- grid.arrange(grobs=histlist[[6]], nrow=1, ncol=4)

if(length(list.dirs(sprintf("%s/Fig2", fd), recursive=F))==0){
  dir.create(sprintf("%s/Fig2", fd))
}

pdf(sprintf("%s/Fig2/Histogram_OTU.pdf", fd), width=8, height=1.5)
plot(OTUL)
dev.off()
