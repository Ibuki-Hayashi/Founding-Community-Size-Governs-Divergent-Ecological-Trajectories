...
objective_function <- function(sigma, real_data){ #- sigma=100
  ...
  sim_data <- forjac(sim_data)
  real_data <- forjac(real_data)
  
  sim_bc <- as.numeric(vegdist(sim_data, method = "jaccard"))
  real_bc <- as.numeric(vegdist(real_data, method = "jaccard"))
...
ksil <- function(df){ #- df <- ldf
  sil <- c(0)
  df <- forjac(df)
  disdf <- vegdist(df, method="jaccard")
  for(i in 2:6){ #- i=3
...
saveRDS(object=SilRess, file=sprintf("%s/Multinomialfir_Master_ACR_jac.rds", fd))
