...
objective_function <- function(sigma, real_data){ #- sigma=100
  ...
  sim_bc <- as.numeric(vegdist(sim_data, method = "hellinger"))
  real_bc <- as.numeric(vegdist(real_data, method = "hellinger"))
...
ksil <- function(df){ #- df <- ldf
  sil <- c(0)
  disdf <- vegdist(df, method="hellinger")
  for(i in 2:6){ #- i=3
...
for(i in 6){ #- i=6
  ...
  for(j in 1:nrow(SilRes)){ #- j=1
    ...
    dis_ldf <- c(vegdist(ldf, method="hellinger"))
...
saveRDS(object=SilRess, file=sprintf("%s/Multinomialfir_Master_ACR_He.rds", fd))
