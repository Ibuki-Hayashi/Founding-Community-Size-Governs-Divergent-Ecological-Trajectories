...
pdf(sprintf("%s/FigS1213/Soil_alphas_384.pdf", fd),width=8,height=6)
su[[3]]; su[[4]]; su[[5]]; su[[6]]; su[[7]]
dev.off()

pdf(sprintf("%s/FigS1213/Water_alphas_384.pdf", fd),width=8,height=6)
wu[[3]]; wu[[4]]; wu[[5]]; wu[[6]]; wu[[7]]
dev.off()

pdf(sprintf("%s/FigS1213/All_alphas_384.pdf", fd),width=8,height=7)
au[[3]]; au[[4]]; au[[5]]; au[[6]]; au[[7]]
dev.off()

pdf(sprintf("%s/FigS1213/ForFig_alphas_384.pdf", fd),width=8,height=7)
au[[6]]
dev.off()
...
#-- output
if(length(list.dirs(sprintf("%s/TableS6", fd), recursive=F))==0){
  dir.create(sprintf("%s/TableS6", fd))
}

write.csv(Ans, sprintf("%s/TableS6/alphas_Statistic_OTU99.csv", fd), row.names=FALSE)
