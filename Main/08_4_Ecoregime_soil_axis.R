...
if(length(list.dirs(sprintf("%s/FigS20soil", fd), recursive=F))==0){
  dir.create(sprintf("%s/FigS20soil", fd))
}

for(m in 1:length(Msegs)){ #- m=1
  folder <- c(list.files("Output/FigS20soil"), "hoge")
  if(any(!(folder %in% sprintf("Minsegs_%s", Msegs[m])))){
    dir.create(sprintf("Output/FigS20soil/Minsegs_%s", Msegs[m]))
  }
  opath <- sprintf("Output/FigS20soil/Minsegs_%s", Msegs[m])
  ...
  for(i in 6){ #- i=6
    ...
    ###########- Max depth
    #- EDR trajectories
    pdf(sprintf("Output/FigS20soil/Minsegs_%s/Trajectories_%s_Depth.pdf", Msegs[m], tax[i]), width=16, height=4.4)
    par(mfrow = c(1, 4))
