...
InoB <- readRDS(sprintf("%s/InoMulti_bray.rds", fd))[[6]]
Sil <- readRDS(sprintf("%s/Multinomialfir_Master_ACR.rds", fd))[[6]]
...
#-- 
InoB$Day <- 0
dff <- as.data.frame(matrix(NA, ncol=ncol(Sil)-ncol(InoB), nrow=nrow(InoB)))
rownames(dff) <- rownames(InoB)
colnames(dff) <- setdiff(colnames(Sil), colnames(InoB))
InoB2 <- cbind(InoB, dff)

uSil <- bind_rows(InoB2, Sil)
...
pdf(sprintf("%s/FigS18/CommMultiDay0_8.pdf", fd), width=3.7, height=2)
plpl
dev.off()
