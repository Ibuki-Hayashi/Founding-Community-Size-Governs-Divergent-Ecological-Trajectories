...
log_likelihood <- function(sigma, observed_data, n, p){ #- n=3000; p=1/3; sigma=130; observed_data=df
  ...
  return(-log_likelihood_value)  # return the negative log-likelihood for optimization
}

#-- Histogram
qthr <- 0.05 #-- qthr = the threshold of q-value for FDR
Readthr <- 3000 #-- Readthr is the threshold of the number of reads
histlist <- list()
for(i in 5:7){ #- i=5
  histlist[[i]] <- gList()
  SilRes <- Sil[[i]]
  for(j in 1:nrow(SilRes)){ #- j=4; j=260
    ...
    if(SilRes$qvalue[j] > qthr){
      ...
      histlist[[i]][[j]] <- fuga1 <- ggplot(data.frame(Sp=ldf), aes(x=Sp))+
        geom_histogram(aes(y=after_stat(density)), bins = 60, fill = "lightblue", color = "black", alpha = 0.6)+
        theme_classic()+ggtitle(sprintf("%s_%s_%s_%s", SilRes$Source[j], SilRes$Density[j], SilRes$Day[j], SilRes$Taxa[j]))+
        theme(text=element_text(size=7),axis.title.y=element_markdown(),
              legend.position="none")+xlab("Relative abundance")+ylab("Frequency")+
        geom_line(data = theory_density, aes(x = x, y = density), color = "red", size = 0.5)
    }else{
      #- kmeans & Silhouette
      kres <- ksil(ldf)
      ...
      for(cl in 1:length(clus)){ #- cl=2
        ...
        if(length(ldf2) > 3){ #- using only the cluster with more than 3 samples
      ...
      histlist[[i]][[j]] <- fuga1 <- ggplot(data.frame(Sp=ldf), aes(x=Sp))+
        ...
        theme(text=element_text(size=6),axis.title.y=element_markdown(),
              legend.position="none")+xlab("Relative abundance")+ylab("Frequency")+
        geom_line(data = theory_density, aes(x = x, y = density), color = "red", size = 0.3)
    }
  }
}
