...
#-- calcurate alphas
alpha_mat <- function(data, enumber, ino){ #- data=data[[6]]; enumber=6; ino="soil"
  seqtab <- data[(data$Source == ino),(enumber+1):ncol(data)]
  exp <- data[(data$Source == ino), 1:enumber]
  
  ans <- data.frame(Shannon2 = diversity(seqtab, index="shannon")/log(rowSums(seqtab>0)),
                    Shannon1 = exp(diversity(seqtab, index="shannon")))
...
box_rich <- function(ddd, title){
  box <- ggplot(ddd, aes(x=Day, y=Shannon1))+
...
box_shannon <- function(ddd, title){
  box <- ggplot(ddd, aes(x=Day, y=Shannon2))+
    geom_violin(aes(group=Day))+ylab("<i>&alpha;</i> diversity of community<br>(Shannon evenness)")+
    geom_jitter(aes(color=Day), size=1)+
    theme_bw()+theme(text=element_text(size=10),axis.title.y=element_markdown(),
                        legend.position="none",
                        strip.background = element_rect(fill = "black"),
...
# -- output
if(length(list.dirs(sprintf("%s/FigS1213", fd), recursive=F))==0){
  dir.create(sprintf("%s/FigS1213", fd))
}

pdf(sprintf("%s/FigS1213/Soil_alphas_384_both_shannonlogS.pdf", fd),width=8,height=6)
su[[3]]; su[[4]]; su[[5]]; su[[6]]; su[[7]]
dev.off()

pdf(sprintf("%s/FigS1213/Water_alphas_384_both_shannonlogS.pdf", fd),width=8,height=6)
wu[[3]]; wu[[4]]; wu[[5]]; wu[[6]]; wu[[7]]
dev.off()

pdf(sprintf("%s/FigS1213/All_alphas_384_both_shannonlogS.pdf", fd),width=8,height=7)
au[[3]]; au[[4]]; au[[5]]; au[[6]]; au[[7]]
dev.off()

pdf(sprintf("%s/FigS1213/ForFig_alphas_384_both_shannonlogS.pdf", fd),width=8,height=7)
au[[6]]
dev.off()