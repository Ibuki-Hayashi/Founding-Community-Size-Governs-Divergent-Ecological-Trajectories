...
for(i in 1:length(tax)){ #- i=4
  ...
  #- Combine unused taxa with Others
  mt <- df[,7:ncol(df)]
  ...
  for(j in 1:length(S_density)){ #- j = the number of density treatment
    ...
    s_plot[[j]] <- leg <- ggplot(sdf)+
      ...
      ggtitle(sprintf("Soil_%s", S_density[j]))+xlab("Replicate communities")+ylab("Relative abundance")+
    ...
    w_plot[[j]] <- leg <- ggplot(wdf)+
      geom_bar(aes(x=Location, y=Abundance, fill=Taxonomy), position="fill", sta="identity", width=0.9)+
      ggtitle(sprintf("Water_%s", S_density[j]))+xlab("Replicate communities")+ylab("Relative abundance")+
      theme(text = element_text(size=7), panel.grid.minor=element_blank(), axis.ticks=element_blank(),
            panel.spacing=unit(1.5,"pt"), plot.margin=unit(c(1,2.5,1,2.5),"pt"),
            panel.grid.major=element_blank(), panel.background=element_blank(), axis.text.x=element_blank(),strip.text.x=element_text(margin=margin(0,0,0,0,unit="pt")))+
      facet_wrap2(.~Day, scales = "free_x", nrow=6, strip=SSS)+scale_y_continuous(expand=c(0,0))+scale_x_discrete(expand=c(0,0))+
  ...
  all[[i]] <- plot_grid(plot_grid(s_ans, w_ans, nrow=2), leg, nrow=1, ncol=2,  rel_widths=c(0.8,width_fig[i]))
}

for(i in 1:length(tax)){ #- i=4
  pdf(sprintf("%s/All_bar_%s.pdf", fd, tax[i]), height=9, width=8)
  plot(all[[i]])
  dev.off()
}
