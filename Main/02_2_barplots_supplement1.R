...
if(length(list.dirs(sprintf("%s/FigS9_10_11", fd), recursive=F))==0){
  dir.create(sprintf("%s/FigS9_10_11", fd))
}

pdf(sprintf("%s/FigS9_10_11/bar_main2_%s.pdf", fd, tax[i]), height=850/25.4, width=700/25.4)
plot(all[[6]])
dev.off()

pdf(sprintf("%s/FigS9_10_11/bar_mainLL2_%s.pdf", fd, tax[i]), height=800/25.4, width=120/25.4)
plot(all_l[[6]])
dev.off()
