...
OTU99_p5 <- grid.arrange(grobs = c(
  unlist(histlist[[6]][161:175], recursive = FALSE),
  cc),
  nrow=8, ncol=5)


if(length(list.dirs(sprintf("%s/FigS15", fd), recursive=F))==0){
  dir.create(sprintf("%s/FigS15", fd))
}

pdf(sprintf("%s/FigS15/OTU99_histo_withModel.pdf", fd), width=8, height=11)
plot(OTU99_p1)
plot(OTU99_p2)
plot(OTU99_p3)
plot(OTU99_p4)
