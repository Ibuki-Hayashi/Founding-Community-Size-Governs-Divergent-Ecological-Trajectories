rm(list = ls(all.names = TRUE))

library(ggplot2); library(vegan);library(gridExtra); library(cowplot);library(lemon); library(cluster)
library(tidyr); library(stringr);library(ggforce); library(ggstar);library(ggnewscale); library(ggh4x)

source("Functions/functions.R")

#-- information of directory
fd <- "Barplot"

folder <- c(list.files("Output"), "hoge")
if(any(!(folder %in% fd))){
  dir.create(sprintf("Output/%s", fd))
}
fd <- sprintf("Output/%s", fd)

dd1 <- "Data"; dd2 <- "Data2"

exp <- read.csv(sprintf("%s/expdata_rep.csv", dd1))
data <- readRDS(sprintf("%s/List_data_rep.rds", dd2)); tax <- c("cla","ord","fam","gen","ASV","OTU99","OTU97")
S_density <- c(1, 0.1, 0.01, 0.001)
W_density <- c(1, 0.1, 0.01, 0.001)

#-- make data ordered, & long data & color palettes
thr1 <- c(8,12,14,18,36,22,28)
thr2 <- c(10,16,20,24,48,30,42)

soil_ans <- list(); water_ans <- list(); all <- list(); all_l <- list()
width_fig <- c(0.15, 0.15, 0.2, 0.4, 0.2, 0.15, 0.15)
legncol <- c(1,1,1,1,2,1,1)
for(i in 1:length(tax)){ #- i=6
  df <- data[[i]]
  df$Day <- sprintf("Day%s", df$Day)
  dfs <- df[df$Source=="soil",]; dfw <- df[df$Source=="water",]
  s_key <- colSums(dfs[,7:ncol(dfs)]); w_key <- colSums(dfw[,7:ncol(dfw)]); a_key <- colSums(df[,7:ncol(df)])
  
  s_tax <- names(s_key)[s_key > sort(s_key, T)[thr1[i]/2]-1]
  w_tax <- names(w_key)[w_key > sort(w_key, T)[thr1[i]/2]-1]
  
  a_key <- a_key[!(names(a_key) %in% union(s_tax, w_tax))]
  a_tax <- names(a_key)[a_key > sort(a_key, T)[thr2[i]-thr1[i]]-1]
  
  u_tax <- unique(c(s_tax, w_tax, a_tax))
  u_tax <- u_tax[!(u_tax %in% c("Unidentified", "Others"))]
  u_tax <- c(u_tax, "Unidentified", "Others")
  cpal <- palette_30[1:length(u_tax[-c(1,2)])]
  cpal <- c(cpal, "#2a333c", "#d3d3d3")
  
  #- Others
  mt <- df[,7:ncol(df)]
  ndf <- cbind(df[,1:6], cbind(mt[, colnames(mt) %in% u_tax], Others=rowSums(mt[, !(colnames(mt) %in% u_tax)])))
  lndf <- gather(ndf, Taxonomy, Abundance, -c(1:6))
  lndf$Taxonomy <- factor(lndf$Taxonomy, levels=u_tax)
  
  #- Location
  dfs_well <- dfs[(dfs$Day=="Day8"), c(7:ncol(dfs))] %>%
    vegdist(method="bray") %>% hclust(method="average")
  sk <- dfs$Location[dfs_well[["order"]]]
  
  dfw_well <- dfw[(dfw$Day=="Day8"), c(7:ncol(dfw))] %>%
    vegdist(method="bray") %>% hclust(method="average")
  wk <- dfw$Location[dfw_well[["order"]]]
  
  s_plot <- list(); w_plot <- list()
  for(j in 1:length(S_density)){ #- j = the number of density treatment
    SSS <- strip_themed(background_x = element_rect(fill=rep("grey5", 6)), text_x = elem_list_text(colour="grey90", size=9*5))
    
    sdf <- lndf[(lndf$Source=="soil")&(lndf$Inoculum_density==S_density[j]),]
    sdf$Location <- factor(sdf$Location, levels=sk)
        
    s_plot[[j]] <- leg <- ggplot(sdf)+
      geom_bar(aes(x=Location, y=Abundance, fill=Taxonomy), position="fill", sta="identity", width=0.9)+
      ggtitle(sprintf("Soil_%s", S_density[j]))+xlab("")+ylab("")+
      theme(text = element_text(size=8*5), panel.grid.minor=element_blank(), axis.ticks=element_blank(),
            panel.spacing=unit(5,"mm"), plot.margin=unit(c(1,2,1,2),"pt"), axis.text.y=element_blank(),
            panel.grid.major=element_blank(), panel.background=element_blank(), axis.text.x=element_blank(),strip.text.x=element_text(margin=margin(0,0,0,0,unit="pt")))+
      facet_wrap2(.~Day, scales = "free_x", nrow=6, strip=SSS)+scale_y_continuous(expand=c(0,0))+scale_x_discrete(expand=c(0,0))+
      scale_fill_manual(values=cpal)
    s_plot[[j]] <- s_plot[[j]]+theme(legend.position = "none")
    
    wdf <- lndf[(lndf$Source=="water")&(lndf$Inoculum_density==W_density[j]),]
    wdf$Location <- factor(wdf$Location, levels=wk)
    
    w_plot[[j]] <- leg <- ggplot(wdf)+
      geom_bar(aes(x=Location, y=Abundance, fill=Taxonomy), position="fill", sta="identity", width=0.9)+
      ggtitle(sprintf("Water_%s", S_density[j]))+xlab("")+ylab("")+
      theme(text = element_text(size=8*5), panel.grid.minor=element_blank(), axis.ticks=element_blank(),
            panel.spacing=unit(5,"mm"), plot.margin=unit(c(1,2,1,2),"pt"), axis.text.y=element_blank(),
            panel.grid.major=element_blank(), panel.background=element_blank(), axis.text.x=element_blank(),strip.text.x=element_text(margin=margin(0,0,0,0,unit="pt")))+
      facet_wrap2(.~Day, scales = "free_x", nrow=6, strip=SSS)+scale_y_continuous(expand=c(0,0))+scale_x_discrete(expand=c(0,0))+
      scale_fill_manual(values=cpal)+guides(fill = guide_legend(ncol = legncol[i]))
    w_plot[[j]] <- w_plot[[j]]+theme(legend.position = "none")
  }
  leg <- g_legend(leg)
  
  s_ans <- arrangeGrob(s_plot[[1]], s_plot[[2]], s_plot[[3]], s_plot[[4]], ncol = 4, nrow = 1,
                   padding = unit(10, "mm"))
  w_ans <- arrangeGrob(w_plot[[1]], w_plot[[2]], w_plot[[3]], w_plot[[4]], ncol = 4, nrow = 1,
                       padding = unit(10, "mm"))
  
  #s_ans <- grid.arrange(s_plot[[1]], s_plot[[2]], s_plot[[3]], s_plot[[4]], nrow=1, ncol=4)
  #w_ans <- grid.arrange(w_plot[[1]], w_plot[[2]], w_plot[[3]], w_plot[[4]], nrow=1, ncol=4)
  
  all[[i]] <- plot_grid(s_ans, w_ans, nrow=2)
  all_l[[i]] <- leg
}

ggsave(all[[6]], filename=sprintf("%s/bar_main2_%s.pdf", fd, tax[i]), height=850, width=800, units="mm")
ggsave(all_l[[6]], filename=sprintf("%s/bar_mainLL2_%s.pdf", fd, tax[i]), height=850, width=120, units="mm")

pdf(sprintf("%s/bar_main2_%s.pdf", fd, tax[i]), height=850/25.4, width=700/25.4)
plot(all[[6]])
dev.off()

pdf(sprintf("%s/bar_mainLL2_%s.pdf", fd, tax[i]), height=800/25.4, width=120/25.4)
plot(all_l[[6]])
dev.off()


