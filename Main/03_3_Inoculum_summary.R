...
for(i in 3:5){ #- i=5
  ...
  Col <- unique(hoge[,2]); Ft <- Col[order(rk, decreasing=TRUE)]
...
if(length(list.dirs(sprintf("%s/FigS8", fd), recursive=F))==0){
  dir.create(sprintf("%s/FigS8", fd))
}

pdf(sprintf("%s/FigS8/Inoculum_bar.pdf", fd), width=7, height=8.5)
Ans
dev.off()