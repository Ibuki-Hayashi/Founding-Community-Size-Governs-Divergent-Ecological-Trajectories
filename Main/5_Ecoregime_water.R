...
dd <- "Data"; fd <- "Output"
dir.create(sprintf("%s/Ecoregime_water", fd))
...
for(m in 1:length(Msegs)){ #- m=1
  folder <- c(list.files(sprintf("%s/Ecoregime_water", fd)), "hoge")
  if(any(!(folder %in% sprintf("Minsegs_%s", Msegs[m])))){
    dir.create(sprintf("%s/Ecoregime_water/Minsegs_%s", fd, Msegs[m]))
  }
  opath <- sprintf("%s/Ecoregime_water/Minsegs_%s", fd, Msegs[m])
  
  #- EDR calculation
  EDR_list <- list(); t_list <- list()
  s_list <- list(); d_list <- list()
  for(i in 4:length(tax)){ #- i=4
  ...
  for(i in 4:length(tax)){ #- i=5
    pdf(sprintf("%s/Ecoregime_water/Minsegs_%s/Trajectories_%s.pdf", fd, Msegs[m], tax[i]), width=16, height=4.4)
    par(mfrow = c(1, 4))
    ...
    write.csv(ans, sprintf("%s/Ecoregime_water/Minsegs_%s/Metrices_%s.csv", fd, Msegs[m], tax[i]))
  }
}
