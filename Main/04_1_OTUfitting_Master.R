...
#-- Multimode test by ACR method fro each OTU
SilRess <- list()
...
for(i in 6){ #- i=6
  ...
  #- Getting inoculum CV for each taxon and adding it to SilRes
  ...
  for(j in 1:nrow(SilRes)){ #- j=1
    #- for monomodal taxa only
    if(SilRes$qvalue[j] > qthr){
      if(SilRes$Source[j] == "Soil"){
        ldf <- LDF <- sTax[[i]][[which(S_density == SilRes$Density[j])]][[which(day == SilRes$Day[j])]]
      }else{
        ldf <- LDF <- wTax[[i]][[which(W_density == SilRes$Density[j])]][[which(day == SilRes$Day[j])]]
      }
      ldf <- ttt <- as.vector(ldf[,colnames(ldf) == SilRes$Taxa[j]])
      kk1 <- LDF$Sample_name[order(ttt)][c(1:outl, (length(ldf)-outl+1):length(ldf))]
      kk2 <- LDF$Location[order(ttt)][c(1:outl, (length(ldf)-outl+1):length(ldf))]
      
      ldf <- sort(ldf)[outl:(length(ldf)-outl)]
      
      #- detect outliers
      adout <- data.frame(rep(SilRes$Source[j], 2*outl), rep(SilRes$Density[j], 2*outl),
                          rep(SilRes$Day[j], 2*outl), rep(SilRes$Taxa[j], 2*outl),
                          kk1, kk2)
      colnames(adout) <- c("Source", "Density", "Day", "Taxa", "Sample_name", "Wellpostition")
      otl[[i]] <- rbind(otl[[i]], adout)
      
      binomp <- mean(ldf)/Readthr #- the p of binomial distribution (n is Readthr)
      #-- optimization
      itv <- c(); itll <- c()
      for(itr in 1:fititr){
  ...
  for(j in 1:nrow(SilRes)){ #- j=96
    ...
    if(SilRes$qvalue[j] <= qthr){
      ...
      for(cl in 1:length(clus)){ #- cl=3
        ...
        if((length(ldf2) > 5)&(var(ldf2)>0.1)){ #- s of the cluster should be more than 5 and the variance of the cluster should be more than 0.1 to fit with binomial distribution with normal error
          binomp <- mean(ldf2)/Readthr #- the p of binomial distribution (n is Readthr)
          #-- optimization
          itv <- c(); itll <- c()
          for(itr in 1:fititr){
...
saveRDS(object=SilRess, file=sprintf("%s/Binomialfir_Master_ACR.rds", fd))

if(length(list.dirs(sprintf("%s/TableS3", fd), recursive=F))==0){
  dir.create(sprintf("%s/TableS3", fd))
}
write.csv(otl[[6]], file=sprintf("%s/TableS3/TableS3.csv", fd), row.names=FALSE, quote=FALSE)
