...
objective_function <- function(sigma, real_data){ #- sigma=100
  ...
  mat_ten <- sqrt(distance(sim_data, method = "jensen-shannon"))
  sim_bc <- mat_ten[upper.tri(mat_ten)]
  
  mat_ten <- sqrt(distance(real_data, method = "jensen-shannon"))
  real_bc <- mat_ten[upper.tri(mat_ten)]
...
ksil <- function(df){ #- df <- ldf
  sil <- c(0)
  mat_ten <- sqrt(distance(df, method = "jensen-shannon"))
  disdf <- mat_ten[upper.tri(mat_ten)]
...
for(i in 6){ #- i=6
  ...
  for(j in 1:nrow(SilRes)){ #- j=1
    ...
    mat_js <- sqrt(distance(ldf, method = "jensen-shannon"))
    dis_ldf <- mat_js[upper.tri(mat_js)]
...
saveRDS(object=SilRess, file=sprintf("%s/Multinomialfir_Master_ACR_JS.rds", fd))
