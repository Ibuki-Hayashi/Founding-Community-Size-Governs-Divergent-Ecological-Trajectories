...
if(length(list.dirs(sprintf("%s/Fig5soil", fd), recursive=F))==0){
  dir.create(sprintf("%s/Fig5soil", fd))
}

for(m in 1:length(Msegs)){ #- m=1
  folder <- c(list.files("Output/Fig5soil"), "hoge")
  if(any(!(folder %in% sprintf("Minsegs_%s", Msegs[m])))){
    dir.create(sprintf("Output/Fig5soil/Minsegs_%s", Msegs[m]))
  }
  opath <- sprintf("Output/Fig5soil/Minsegs_%s", Msegs[m])
  ...
  for(i in 6){ #- i=6
    ###########- Max size
    #- EDR trajectories
    pdf(sprintf("Output/Fig5soil/Minsegs_%s/Trajectories_%s_Size.pdf", Msegs[m], tax[i]), width=16, height=4.4)
    par(mfrow = c(1, 4))
    ...
    write.csv(ans, sprintf("Output/Fig5soil/Minsegs_%s/Metrices_%s_Size.csv", Msegs[m], tax[i]))
    
    ###########- Min Average Link
    #- EDR trajectories
    pdf(sprintf("Output/Fig5soil/Minsegs_%s/Trajectories_%s_Link.pdf", Msegs[m], tax[i]), width=16, height=4.4)
    par(mfrow = c(1, 4))
    ...
    write.csv(ans, sprintf("Output/Fig5soil/Minsegs_%s/Metrices_%s_Link.csv", Msegs[m], tax[i]))
    
    ###########- Max density
    #- EDR trajectories
    pdf(sprintf("Output/Fig5soil/Minsegs_%s/Trajectories_%s_Density.pdf", Msegs[m], tax[i]), width=16, height=4.4)
    par(mfrow = c(1, 4))
    for(j in 1:length(S_density)){
      summ <- summary(EDR_list[[i]][[j]])
    ...
    write.csv(ans, sprintf("Output/Fig5soil/Minsegs_%s/Metrices_%s_Density.csv", Msegs[m], tax[i]))
    
    ###########- Max depth
    #- EDR trajectories
    pdf(sprintf("Output/Fig5soil/Minsegs_%s/Trajectories_%s_Depth.pdf", Msegs[m], tax[i]), width=16, height=4.4)
    par(mfrow = c(1, 4))
    for(j in 1:length(S_density)){
      summ <- summary(EDR_list[[i]][[j]])
    ...
    write.csv(ans, sprintf("Output/Fig5soil/Minsegs_%s/Metrices_%s_Depth.csv", Msegs[m], tax[i]))
    
    ###########- Max rank
    #- EDR trajectories
    pdf(sprintf("Output/Fig5soil/Minsegs_%s/Trajectories_%s_Rank.pdf", Msegs[m], tax[i]), width=16, height=4.4)
    par(mfrow = c(1, 4))
    for(j in 1:length(S_density)){ #- j=1
      summ <- summary(EDR_list[[i]][[j]])
    ...
    write.csv(ans, sprintf("Output/Fig5soil/Minsegs_%s/Metrices_%s_Rank.csv", Msegs[m], tax[i]))
  }
}
