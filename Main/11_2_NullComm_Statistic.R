...
for(i in 5:7){ #- i=6
  ...
  Cont_S <- ggplot(uSilS, aes(x=Day, y=sigma))+
    xlab("Day")+ylab("Variation caused after<br>inoculation in community-level")+theme_bw()+
  ...
  plpl[[i]] <- plot_grid(plot_grid(Cont_S, Cont_W, Dist_S, Dist_W, ncol=4, nrow=1), cl, ncol=2, rel_widths=c(1, 0.1))
}

pdf(sprintf("%s/NullComm_plot.pdf", fd), width=8, height=7.2)
pl[[5]]
pl[[6]]
pl[[7]]
dev.off()

pdf(sprintf("%s/NullComm_plot_forfig.pdf", fd), width=8, height=2)
plpl[[6]]
dev.off()
