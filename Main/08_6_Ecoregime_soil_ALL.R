...
if(length(list.dirs(sprintf("%s/Fig4EDRsoil", fd), recursive=F))==0){
  dir.create(sprintf("%s/Fig4EDRsoil", fd))
}

for(m in 1:length(Msegs)){ #- m=1
  folder <- c(list.files("Output/Fig4EDRsoil"), "hoge")
  if(any(!(folder %in% sprintf("Minsegs_%s", Msegs[m])))){
    dir.create(sprintf("Output/Fig4EDRsoil/Minsegs_%s", Msegs[m]))
  }
  opath <- sprintf("Output/Fig4EDRsoil/Minsegs_%s", Msegs[m])
  ...
  for(i in 6){ #- i=6
    ...
    pdf(sprintf("Output/Fig4EDRsoil/Minsegs_%s/Trajectories_%s_All.pdf", Msegs[m], tax[i]), width=8, height=8)
