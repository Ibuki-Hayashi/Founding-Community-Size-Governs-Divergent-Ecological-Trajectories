...
#-- Save
if(length(list.dirs(sprintf("%s/FigS17", fd), recursive=F))==0){
  dir.create(sprintf("%s/FigS17", fd))
}

ans_99 <- grid.arrange(grobs=pls[[6]], ncol=5, nrow=7)

pdf(sprintf("%s/FigS17/Histo_Bray_withModel_OTU99.pdf", fd), width=8, height=11.5)
plot(ans_99)
dev.off()
