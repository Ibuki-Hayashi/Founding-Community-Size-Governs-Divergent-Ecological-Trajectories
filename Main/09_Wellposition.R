...
day <- c(2, 4, 6, 8)
scal <- 1.5 #- scaling factor for figure size
...
#-- kmeans & Silhouette
ksil <- function(df){ #- df <- df[,7:ncol(df)]
  sil <- c(0)
...
#-- Clustering
Figin <- function(df, scaling=scal){ #- df = sTax[[6]][[1]][[1]]
  mat <- df[,7:ncol(df)]
  ks <- ksil(mat)
...
if(length(list.dirs(sprintf("%s/FigS24_25", fd), recursive=F))==0){
  dir.create(sprintf("%s/FigS24_25", fd))
}

ggsave(S_day8, filename=sprintf("%s/FigS24_25/OTU99_Soil_cluster.pdf", fd), height=1100*scal/5, width=800*scal/5, units="mm")
ggsave(W_day8, filename=sprintf("%s/FigS24_25/OTU99_Water_cluster.pdf", fd), height=1100*scal/5, width=800*scal/5, units="mm")
