...
Sil <- readRDS(sprintf("%s/Multinomialfir_Master_ACR_He.rds", fd))
...
if(length(list.dirs(sprintf("%s/FigS7", fd), recursive=F))==0){
  dir.create(sprintf("%s/FigS7", fd))
}

pdf(sprintf("%s/FigS7/NullComm_plot_He.pdf", fd), width=8, height=2)
plpl[[6]]
dev.off()
