...
ksil <- function(df){ #- df <- ldf
  ...
  for(i in 2:6){ #- i=3
    ...
    sil[i] <- mean(s[, 3])  # the average silhouette score
  }
...
for(i in 6){ #- i=6
  SilRes <- Sil[[i]]
  SilRes <- SilRes[(SilRes$Source == "Soil")&(SilRes$Day == 4),]
  pls[[i]] <- list()
  for(jj in 1:nrow(SilRes)){ #- jj=2
    ...
    if(SilRes$qvalue[jj] > qthr){
      ...
      pls[[i]][[jj]] <- ggplot(df)+
        geom_histogram(aes(x=real_bc), bins=60, fill="tomato", alpha=0.7)+
        geom_histogram(aes(x=sim_bc), bins=60, fill="lightblue", alpha=0.7)+
...
ans_99 <- grid.arrange(grobs=pls[[6]], ncol=4, nrow=1)

pdf(sprintf("%s/Fig3/Histo_Fig3.pdf", fd), width=8, height=1.2)
plot(ans_99)
dev.off()
