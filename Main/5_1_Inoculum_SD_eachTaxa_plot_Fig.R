...
for(i in 6){ #- i=6
  ...
  for(j in 1:length(unique(uSil$unique))){ #- j=1
    ...
    if(sum(key > 2) >= 2){
      corpl1[[i]][[itr]] <- ggplot(df, aes(x=Ino_CV, y=CV, color=as.factor(Day)))+
        geom_point(size=1.3)+geom_line(size=0.25)+theme_bw()+xlab("Initial variation<br>upon inoculation")+ylab("Within mode<br>variation")+
  ...
  topCD <- ggplot(uSil)+
    ...
    xlab("Within mode<br>variation")+ylab("Multimodality")+
    ggtitle("")+theme(text=element_text(size=7.2), axis.title=element_markdown())+
    scale_color_manual(values=Daycol)
  
  cd2 <- ggplot(uSil[uSil$Day == 2,])+
    geom_point(aes(x=CV, y=Statistic, color=Density), size=0.8)+theme_bw()+
    xlab("Within mode<br>variation")+ylab("Multimodality")+scale_color_manual(values=Inocol)+
